...
in the first (and only, since we can't exploit the above construction) trial. Now, say we can break Poseidon. We obtain some **useful** collisions and after that substitute `MMTs` with `FHMTs`. Since we forged our trees after random challenges, we could try to pass a proof where our Merkle proofs and commitments completely mirrors honest ones, **but we send malicious statediffs in order to make everyone update their states, then control the root and think _"ok the proof is safe"_ (while it isn't)**.

Gemini replied that the previous scenario could be prevented nonethelss if the verifier inserts `statediffs` data directly from blobs into AIR constraints. This is true only if our malicious `statediffs` are completely unrelated to our `FHMTs`. If, instead, we manipulate statediffs to be fake honest in the places corresponding to the random challenges (that is, the trace lines controlled by the verifier) then we should be able to pass a malicious Merkle tree.
